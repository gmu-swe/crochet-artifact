...
For the microbenchmarks table (Table 1): The ``normal'' set of columns are the runtime in nanoseconds; ``crij'' is the relative slowdown of \sysname, ``crijcb'' is the relative slowdown of \sysname with checkpoint, ``cloner'' is the relative slowdown of the deepcloner system, and the last 3 columns are the relative slowdown of CRIU.

For the DaCapo table (Table 2): The ``normal'' set of columns are the runtime in milliseconds, ``crij'' is the relative slowdown of \sysname, ``crijcb'' is the relative slowdown of \sysname with checkpoint called, and ``criu'' is the relative slowdown of CRIU.

For the STM table (Table 3): The ``normal'' set of columns are the number of completed operations in the baseline, ``notx,'' ``onetx'' and ``manytx'' correspond to the next three sets of columns in the table (percent of operations completed).
...
For the FTP table (Table 5): The first three columns represent the execution time in milliseconds. The next three columns represent the relative slowdown.

For the result in 6.2 on Dacapo-H2: the dacapoh2 generated table contains the average time (and standard deviation) for the baseline (``normal''), \sysname without checkpoint (``crochet-no-checkpoint''), and using \sysname to do the checkpoint and rollback (``crochet'').
...
state, as explained in Section~4.1.
...
As another example, methods \texttt{cp.onCheckpoint} and
\texttt{rb.onCheckpoint} both map to the same method \texttt{slowOnCheckpoint}.
Depending on whether the version is even or odd, the prototype decides to run
\texttt{cp.onCheckpoint}, on line 734; or the \texttt{rb.onCheckpoint}, on line
726.  Note that the CAS on line 52 is not needed as both \texttt{CHECK} and
